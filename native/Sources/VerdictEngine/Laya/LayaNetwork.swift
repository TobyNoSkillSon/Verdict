...
private final class LayaGraph {
    ...
    init(config: LayaEncoderConfiguration, headLayers: Int, arrays: [String: MLXArray], linears: [String: Linear]) {
        self.config = config; self.headLayers = headLayers; self.arrays = arrays; self.linears = linears
    }
    ...
    func attention(_ x: MLXArray, name: String, heads: Int, mask: MLXArray, ropeBase: Float? = nil) -> MLXArray {
    ...
    func forward(_ inputs: [MLXArray]) -> MLXArray {
        ...
        let local = logicalAnd(logicalOr(localDistances.reshaped(1, 1, length, length), logicalNot(valid.reshaped(b, 1, length, 1))), full)
        ...
        for i in 0..<config.num_hidden_layers {
            ...
            x = x + attention(normalized, name: prefix + ".attn", heads: config.num_attention_heads, mask: kind == "full_attention" ? full : local, ropeBase: config.ropeBase(kind))
