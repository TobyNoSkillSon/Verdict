...
4. **Verify.** `verdict status` should list a model with `(Optimized · <chip>)` (Verdict's optimized path passed its self-test on this Mac) or `(MLX: <reason>)` (not fully optimized — the stock path or one optimization off: it works, more slowly). Then:
